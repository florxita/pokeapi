...
const traerPokemones = async () => {
  try {
    for (let i = 0; i < 3; i++) {
      // solo me va a traer 3/150 pokemones
      ...
      let respColor = await fetch(
        `https://pokeapi.co/api/v2/pokemon-species/${pokeRandom}/`
      );
      let dataColor = await respColor.json();

      const pokemon = {
        // creo el objeto pokemon, solo img y nombre
...
const pintarNombrePokemon = () => {
  ...
  pokemones.forEach((pokemon) => {
    let upperName =
      pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1); // 1er letra Mayusc.
    form.innerHTML += `
  ...
  arrayLabel.forEach((label) => {
    // recorro los labels para darle/quitar la clase css = seleccionado
